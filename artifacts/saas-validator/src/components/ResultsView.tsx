...
/* ── Score card ─────────────────────────────────────────── */
function ScoreCard({ score, label, sublabel, color, explanation }: {
  score: number; label: string; sublabel: string; color: string; explanation?: string;
}) {
  return (
    <div className="rounded-2xl border p-5 flex flex-col items-center text-center gap-3"
...
/* ── Verdict banner ─────────────────────────────────────── */
function VerdictBanner({ badge, explanation, verdictLabel }: {
...
function SourceRow({ source }: { source: Source }) {
  ...
  return (
    <div className="flex items-start gap-3 py-3 border-b last:border-0"
      ...
      <p className="flex-1 text-sm leading-relaxed" style={{ color: "#94a3b8" }}>{source.finding}</p>
      <span className="w-2 h-2 rounded-full flex-shrink-0 mt-1.5"
        style={{ backgroundColor: dot }} title={source.sentiment} />
    </div>
  );
}
...
function CompetitorCard({ competitor }: { competitor: Competitor }) {
  return (
    <div className="rounded-xl border p-4 mb-3 last:mb-0"
      style={{ backgroundColor: "rgba(255,255,255,0.02)", borderColor: "rgba(255,255,255,0.07)" }}>
...
export interface ResultsViewProps {
  ...
  saasName: string;
  niche: string;
  /** Called when the user clicks "back" — if omitted, the button is hidden */
  onBack?: () => void;
...
export default function ResultsView({
  ...
  const badge = getBadge(result.willingness_to_pay, result.market_saturation);
  ...
  const verdictLabel = (verdictLabels[lang as "fr" | "en" | "es"] ?? verdictLabels.fr)[badge];
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <span className="inline-block mt-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium border"
      ...
      <div className="grid grid-cols-2 gap-4">
        <ScoreCard score={result.willingness_to_pay} label={t("results.ring1")} sublabel={t("results.ring1sub")}
          color="#22c55e" explanation={result.willingness_to_pay_explanation} />
        <ScoreCard score={result.market_saturation} label={t("results.ring2")} sublabel={t("results.ring2sub")}
          color="#f97316" explanation={result.market_saturation_explanation} />
      </div>
      ...
      <SectionFlat title={t("results.paidCompetitors")}>
        <div className="flex flex-wrap gap-2">
          {competitors.map((c) => (
            <span key={getCompetitorName(c)} className="px-3 py-1 rounded-lg text-xs font-semibold border"
        ...
        <ul className="space-y-3">
          {result.reddit_insights.map((ins, i) => (
            <li key={i} className="flex items-start gap-3 text-sm" style={{ color: "#94a3b8" }}>
              <span className="mt-0.5 flex-shrink-0 w-5 h-5 rounded-md flex items-center justify-center text-xs font-bold"
      ...
      >
        {result.sources && result.sources.length > 0 ? (
          <div className="divide-y" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
            {result.sources.map((s) => <SourceRow key={s.name} source={s} />)}
          </div>
        ) : (
